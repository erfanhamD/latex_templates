...
% به منظور فارسی شدن اعداد فوتنوت‌های چپ به راست انگلیسی به شرطی که از این گزینه در استفاده از پکیج زیپرشین استفاده شود extrafootnotefeatures
\LTRfootmarkstyle{\textsuperscript{\setpersianfont #1}}
% به منظور اضافه کردن فووت نوت به تایتل هر سکشن و زیر سکشن
...
\newcommand{\دینا}{\lr{DNA} }
\newcommand{\پیسیر}{\lr{PCR} }
\newcommand{\نانوم}{$nm$ }
\newcommand{\میکروم}{$\mu m$ }
\newcommand{\میکرول}{$\mu l$ }
\newcommand{\میلیم}{$mm$ }
\newcommand{\متر}{$m$ }
\newcommand{\درجهسا}{$\,^{\circ}\mathrm{C}$}
\newcommand{\اع}[1]{$#1$}
% -------------------- Dictionary --------------------
...
{\begin{figure}[h!]
		\centering
		\includegraphics[width = 300pt, height = 250pt]{#1}
		\caption{#2}
		\label{#3}
	\end{figure}
	\FloatBarrier
...
\newcommand{\centerimg}[3]
	{\vspace{1em}\begin{center}\includegraphics[scale=#2]{figs/#1}\caption{#3}\end{center}\vspace{-1.5em}}

\newcommand{\figureb}[4]
	{\vspace{1em}\begin{figure}[ht]\centering\includegraphics[scale=#1]{figs/#2}\caption{#3}\ifthenelse{\isempty{#4}}{}{\label{#4}}\end{figure}\vspace{1em}}
